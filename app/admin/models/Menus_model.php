...
class Menus_model extends Model
{
    ...
    public $relation = [
        ...
        'hasOne' => [
            ...
            'point' => ['Admin\Models\Menus_points_model', 'delete' => TRUE],
        ],
        'belongsTo' => [
    ...
    protected $purgeable = ['special', 'point','menu_options', 'categories', 'locations'];
    ...
    protected function afterSave()
    {
        ...
        if (array_key_exists('point', $this->attributes))
            $this->addMenuPoint((array)$this->attributes['point']);
    ...
    public function addMenuPoint(array $menuPoint = [])
    {
        $menuId = $this->getKey();
        if (!is_numeric($menuId) OR !isset($menuPoint['menu_point_id']))
            return FALSE;

        $menuPoint['menu_id'] = $menuId;
        $this->point()->updateOrCreate([
            'menu_point_id' => $menuPoint['menu_point_id'],
        ], array_except($menuPoint, 'menu_point_id'));
    }
}